...
import { formatWeight, formatRupiah, formatDate } from "@/lib/format";
        ...
        {(data || []).map((p) => (
          <Card key={p.id} data-testid={`production-${p.id}`} className="p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="font-semibold">{p.source_name} · Input {formatWeight(p.input_weight)}</p>
...
function ProductionDialog({ source, outs, onClose, onSaved }) {
  ...
  const totalOut = outputs.reduce((s, o) => s + Number(o.weight), 0);
  const susut = Number(f.input_weight) - totalOut;

  const save = async () => {
    if (!f.source_product_id || !Number(f.input_weight)) return toast.error("Lengkapi sumber & input");
    const valid = outputs.filter((o) => o.product_id && Number(o.weight) > 0);
    if (!valid.length) return toast.error("Tambahkan output");
    setBusy(true);
    try {
      await api.post("/productions", {
        source_product_id: f.source_product_id, input_weight: Number(f.input_weight),
        labor_cost: Number(f.labor_cost), packaging_cost: Number(f.packaging_cost), other_cost: Number(f.other_cost),
        operator: f.operator, outputs: valid.map((o) => ({ product_id: o.product_id, weight: Number(o.weight) })),
      });
      ...
      <DialogContent className="bg-popover max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label className="text-xs">Berat Input (kg)</Label><Input data-testid="prod-input" type="number" value={f.input_weight} onChange={(e) => set("input_weight", e.target.value)} className="mt-1 tabular" /></div>
          </div>
          <div className="space-y-2">
            ...
            {outputs.map((o, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-center">
                ...
                <div className="col-span-4"><Input placeholder="kg" type="number" value={o.weight} onChange={(e) => setOut(i, "weight", e.target.value)} className="tabular" /></div>
          ...
          <div className="rounded-lg bg-accent p-3 text-sm flex justify-between tabular">
            <span>Total Output: {formatWeight(totalOut)}</span>
            <span className="text-warning">Susut: {formatWeight(susut > 0 ? susut : 0)}</span>
          </div>
        </div>
